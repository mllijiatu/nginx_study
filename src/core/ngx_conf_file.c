...
/*
 * 向Nginx配置解析上下文中添加配置文件的转储信息
 * 如果文件名已存在于配置转储的红黑树中，则直接返回成功
 * 否则，将文件名复制到内存池中，创建一个 ngx_conf_dump_t 结构体，并插入到配置转储数组和红黑树中
 * 参数：
 *   - cf: Nginx配置解析上下文结构体指针
 *   - filename: 配置文件名
...
static ngx_int_t ngx_conf_add_dump(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    if (sn)
    {
        ...
        return NGX_OK; // 文件名已存在，直接返回成功
    }

    p = ngx_pstrdup(cf->cycle->pool, filename); // 复制文件名到内存池中
    if (p == NULL)
    {
        return NGX_ERROR; // 复制失败，返回错误
    }

    cd = ngx_array_push(&cf->cycle->config_dump); // 向配置转储数组中添加新的元素
    if (cd == NULL)
    {
        return NGX_ERROR; // 添加失败，返回错误
    }

    size = ngx_file_size(&cf->conf_file->file.info);

    buf = ngx_create_temp_buf(cf->cycle->pool, (size_t)size); // 创建临时缓冲区
    if (buf == NULL)
    {
        return NGX_ERROR; // 创建失败，返回错误
    }
    ...
    sn = ngx_palloc(cf->temp_pool, sizeof(ngx_str_node_t)); // 创建红黑树节点
    if (sn == NULL)
    {
        return NGX_ERROR; // 创建失败，返回错误
    }
    ...
    ngx_rbtree_insert(&cf->cycle->config_dump_rbtree, &sn->node); // 插入到红黑树中
...
/*
 * 解析Nginx配置文件
 * 该函数根据文件名打开配置文件，创建配置解析上下文，然后根据配置文件中的指令逐行解析
 * 参数：
 *   - cf: Nginx配置解析上下文结构体指针
 *   - filename: 配置文件名
...
char *ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    for (;;)
    {
        ...
        /*
         * ngx_conf_read_token() 可能返回
         *
         *    NGX_ERROR             解析错误
         *    NGX_OK                发现以 ";" 结束的标记
         *    NGX_CONF_BLOCK_START  发现以 "{" 结束的标记
         *    NGX_CONF_BLOCK_DONE   发现 "}" 标记
         *    NGX_CONF_FILE_DONE    配置文件解析完成
         */
        ...
        if (cf->handler)
        {
            ...
            /*
             * 定制的处理函数，例如，在 http 模块的 "types { ... }" 指令中使用的处理函数
             */
