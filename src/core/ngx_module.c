...
/*
 * 初始化Nginx运行周期中的模块列表
 * 该函数创建一个包含静态模块的模块列表，并将其赋值给当前运行周期的modules字段
 * 参数：
 *   - cycle: Nginx运行周期结构体指针
...
ngx_int_t ngx_cycle_modules(ngx_cycle_t *cycle) {
    ...
    cycle->modules_n = ngx_modules_n;  // 记录静态模块的数量
...
/*
 * 初始化Nginx启动时加载的模块
 * 该函数遍历当前运行周期的模块列表，对每个模块调用其初始化模块回调函数
 * 参数：
 *   - cycle: Nginx运行周期结构体指针
 * 返回 NGX_OK 表示成功，NGX_ERROR 表示失败
 */
    ...
    for (i = 0; cycle->modules[i]; i++) {
        if (cycle->modules[i]->init_module) {
            if (cycle->modules[i]->init_module(cycle) != NGX_OK) {
                return NGX_ERROR;  // 模块初始化失败，返回错误
            }
        }
    }

    return NGX_OK;  // 所有模块初始化成功
}
