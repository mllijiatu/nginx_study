 ...
 * @brief 初始化CRC32表
 *
 * @return 返回初始化状态，NGX_OK 表示成功，NGX_ERROR 表示失败
 */
ngx_int_t ngx_crc32_table_init(void)
{
    ...
    /**
     * @brief 如果 ngx_crc32_table_short 已经对齐到缓存行边界，则无需再次初始化，直接返回成功
     */
    ...
    /**
     * @brief 分配内存用于存储对齐后的 CRC32 表
     *        分配的内存包括 16 个 uint32_t 大小的 CRC32 表以及缓存行大小的对齐空间
     */
    p = ngx_alloc(16 * sizeof(uint32_t) + ngx_cacheline_size, ngx_cycle->log);
    if (p == NULL) {
        return NGX_ERROR;  // 分配失败，返回错误状态
    }

    /**
     * @brief 将分配的内存地址对齐到缓存行边界
     */
    p = ngx_align_ptr(p, ngx_cacheline_size);

    /**
     * @brief 复制标准的 16 个 uint32_t CRC32 表到对齐后的内存中
     */
    ngx_memcpy(p, ngx_crc32_table16, 16 * sizeof(uint32_t));

    /**
     * @brief 将 ngx_crc32_table_short 指向新的对齐后的 CRC32 表
     */
    ngx_crc32_table_short = p;

    return NGX_OK;  // 初始化成功，返回成功状态
}
