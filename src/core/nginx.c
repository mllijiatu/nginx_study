...
/*
 * 静态数组 ngx_core_commands 存储Nginx核心模块的配置指令
 */
static ngx_command_t ngx_core_commands[] = {

    { ngx_string("daemon"),                 // 指令名称
      NGX_MAIN_CONF | NGX_DIRECT_CONF | NGX_CONF_FLAG,  // 指令类型和作用域
      ngx_conf_set_flag_slot,               // 设置指令值的回调函数
      0,                                    // 指令所属的配置块类型
      offsetof(ngx_core_conf_t, daemon),    // 指令值在配置结构中的偏移量
      NULL },                               // 钩子函数，暂时为空
...
/**
 * @brief 主函数入口，负责初始化并启动程序
 *
 * @param argc 参数个数
 * @param argv 参数数组
 * @return 返回程序执行状态，通常为 0 表示成功
 */
int ngx_cdecl main(int argc, char *const *argv)
{
    ngx_buf_t        *b;           // 缓冲区指针
    ngx_log_t        *log;         // 日志指针
    ngx_uint_t        i;           // 无符号整数变量
    ngx_cycle_t      *cycle, init_cycle;  // ngx_cycle_t 结构体指针，用于存储进程周期信息
    ngx_conf_dump_t  *cd;          // 存储配置信息的结构体指针
    ngx_core_conf_t  *ccf;          // 存储核心配置信息的结构体指针

    ngx_debug_init();  // 初始化调试功能

    /**
     * @brief 初始化错误信息处理，若失败则返回 1
     */
    ...
    /**
     * @brief 如果设置了显示版本信息，则显示并根据配置决定是否退出
     */
    if (ngx_show_version) {
    ...
    ngx_time_init();  // 初始化时间相关信息

#if (NGX_PCRE)
    ngx_regex_init();  // 初始化正则表达式
#endif

    ngx_pid = ngx_getpid();       // 获取当前进程ID
    ngx_parent = ngx_getppid();   // 获取当前父进程ID

    log = ngx_log_init(ngx_prefix, ngx_error_log);  // 初始化日志
    if (log == NULL) {
        return 1;
    }
    ...
    ngx_memzero(&init_cycle, sizeof(ngx_cycle_t));  // 清零 init_cycle 结构体
    init_cycle.log = log;
    ...
    if (ngx_process_options(&init_cycle) != NGX_OK) {  // 处理命令行参数
        return 1;
    }

    if (ngx_os_init(log) != NGX_OK) {  // 初始化操作系统相关信息
        return 1;
    }
    ...
    ngx_slab_sizes_init();  // 初始化slab内存分配

    if (ngx_add_inherited_sockets(&init_cycle) != NGX_OK) {  // 处理继承的套接字
        return 1;
    }

    if (ngx_preinit_modules() != NGX_OK) {  // 预初始化模块
        return 1;
    }

    cycle = ngx_init_cycle(&init_cycle);  // 初始化周期信息
    if (cycle == NULL) {
        if (ngx_test_config) {
    ...
    if (ngx_test_config) {  // 如果处于测试配置模式，输出测试成功信息，并在需要时输出配置信息
        if (!ngx_quiet_mode) {
    ...
    if (ngx_signal) {
        return ngx_signal_process(cycle, ngx_signal);  // 处理信号
    }

    ngx_os_status(cycle->log);  // 打印操作系统状态

    ngx_cycle = cycle;  // 设置全局当前周期变量

    ccf = (ngx_core_conf_t *) ngx_get_conf(cycle->conf_ctx, ngx_core_module);  // 获取核心配置信息

    if (ccf->master && ngx_process == NGX_PROCESS_SINGLE) {
        ngx_process = NGX_PROCESS_MASTER;  // 设置进程类型
    }

#if !(NGX_WIN32)

    if (ngx_init_signals(cycle->log) != NGX_OK) {  // 初始化信号处理
        return 1;
    }

    if (!ngx_inherited && ccf->daemon) {
        if (ngx_daemon(cycle->log) != NGX_OK) {  // 如果非继承且需要守护进程化，则执行守护进程化
            return 1;
        }
    ...
    if (ngx_log_redirect_stderr(cycle) != NGX_OK) {  // 重定向stderr输出
        return 1;
    }

    if (log->file->fd != ngx_stderr) {
        if (ngx_close_file(log->file->fd) == NGX_FILE_ERROR) {  // 关闭日志文件描述符
    ...
    if (ngx_process == NGX_PROCESS_SINGLE) {
        ngx_single_process_cycle(cycle);  // 单进程循环

    } else {
        ngx_master_process_cycle(cycle);  // 主进程循环
    }
...
/**
 * @brief 从环境变量中获取继承的套接字并添加到cycle的listening数组中
 *
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    // 从环境变量中获取NGINX_VAR对应的值
    inherited = (u_char *) getenv(NGINX_VAR);

    // 如果环境变量中没有NGINX_VAR，则直接返回成功
    if (inherited == NULL) {
        return NGX_OK;
    }

    // 打印日志，表示正在使用从环境变量中继承的套接字
    ngx_log_error(NGX_LOG_NOTICE, cycle->log, 0,
                  "using inherited sockets from \"%s\"", inherited);

    // 初始化cycle的listening数组
    ...
    // 遍历从环境变量中获取的字符串，解析套接字信息并添加到listening数组中
